...
function brentq(
  ...
  if (!(fa * fb < 0)) {
    throw new Error(
      'Root not bracketed: f(a) and f(b) must have opposite signs'
    )
  }
...
function secular(pdf: PDF): number {
  ...
  const f = (x: number) =>
    pdf.reduce((sum, [ai, pi]) => sum + (pi * ai) / (1 + x * ai), 0)
...
function MLE_t_value(pdfhat: PDF, ref: number, s: number): PDF {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const maxDiff = Math.max(
      ...pdf_MLE.map(([, pi], j) => Math.abs(pi - prev[j]))
    )
...
function LLR_core(
  ...
  s1: number,
  ref: number | null,
  statistic: 'expectation' | 't_value'
): number {
  let pdf0: PDF, pdf1: PDF

  if (statistic === 't_value') {
    if (ref === null)
      throw new Error('ref must be provided for t_value statistic')
...
function results_to_pdf(results: number[]): { N: number; pdf: PDF } {
  ...
  const pdf: PDF = regularizedResults.map((res, i) => [
    i / (count - 1),
    res / N,
  ])
  return { N, pdf }
 ...
 */
export function calculateLLR_normalized(
  nelo0: number,
  nelo1: number,
  results: number[]
): number | null {
 ...
 */
export function calculateLLR_logistic(
  elo0: number,
  elo1: number,
  results: number[]
): number | null {
 ...
 */
export function calculateLLR_normalized_alt(
  nelo0: number,
  nelo1: number,
  results: number[]
): number | null {
  try {
    ...
    return (
      (games / 2) *
      Math.log((1 + (nt - nt0) * (nt - nt0)) / (1 + (nt - nt1) * (nt - nt1)))
    )
  } catch (e) {
 ...
 */
export function summarizeResults(results: number[]): {
  mu: number
  variance: number
  sigma_pg: number
  games: number
