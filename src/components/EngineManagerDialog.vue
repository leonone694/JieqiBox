<template>
  ...
  <script setup lang="ts">
    ...
    const confirmDelete = () => {
      ...
      console.log(`[DEBUG] EngineManager: Deleting engine: ${engineToDelete.value.name}`)
      engines.value = engines.value.filter(e => e.id !== engineToDelete.value!.id)
      console.log(`[DEBUG] EngineManager: Engines remaining: ${engines.value.length}`)
      saveEnginesToConfig()
      ...
      if (lastSelectedId === engineToDelete.value.id) {
        console.log(`[DEBUG] EngineManager: Deleted engine was the last selected one, clearing last selected engine ID`)
      ...
      if (isEditing.value) {
        ...
        if (index > -1) {
          ...
          if (oldEngine.id !== editedEngine.value.id) {
            console.log(`[DEBUG] EngineManager: Engine ID changed from ${oldEngine.id} to ${editedEngine.value.id}, clearing last selected engine ID`)
            configManager.clearLastSelectedEngineId()
          }
        }
      } else {
    ...
    onMounted(async () => {
      ...
      if (engines.value.length === 0) {
        console.log(`[DEBUG] EngineManager: Engine list is empty on mount, clearing last selected engine ID`)
        configManager.clearLastSelectedEngineId()
      }
    ...
    watch(isVisible, newValue => {
      if (newValue) {
        ...
        // Clear last selected engine ID if the engine list is empty
        if (engines.value.length === 0) {
          console.log(`[DEBUG] EngineManager: Engine list is empty when dialog opened, clearing last selected engine ID`)
          configManager.clearLastSelectedEngineId()
        }
      }
    })
