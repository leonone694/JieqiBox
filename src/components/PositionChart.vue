...
const chartData = computed(() => {
  ...
  props.history.forEach((entry, index) => {
    if (entry.type === 'move') {
      ...
      if (convertedScore !== null && convertedScore !== undefined && !isRedMove) {
      ...
      data.push({
        ...
        moveNumber,
        moveText,
        score: convertedScore !== null && convertedScore !== undefined ? convertedScore : null,
...
const moveLabels = computed(() => {
  ...
  const xStep = (chartWidth.value - padding.left - padding.right) / Math.max(1, dataPoints.length - 1);
...
const drawChart = () => {
  ...
  // Find score range
  const scores = dataPoints.map(p => p.score).filter(s => s !== null) as number[];
  if (scores.length === 0) {
    ...
    ctx.fillText(t('positionChart.noData') || 'No analysis data available', chartArea.x + chartArea.width / 2, chartArea.y + chartArea.height / 2);
    return;
  }
  ...
  // Draw score line
  drawScoreLine(ctx, chartArea, dataPoints, displayMinScore, displayScoreRange);
  
  // Draw data points
  drawDataPoints(ctx, chartArea, dataPoints, displayMinScore, displayScoreRange);
};
...
// Draw score axis
const drawScoreAxis = (ctx: CanvasRenderingContext2D, chartArea: any, minScore: number, maxScore: number) => {
  ctx.fillStyle = '#666';
  ctx.font = '12px Arial';
  ctx.textAlign = 'right';
  ctx.textBaseline = 'middle';
  ...
  for (let i = 0; i <= gridLines; i++) {
    ...
    const y = chartArea.y + chartArea.height - ((score - minScore) / (maxScore - minScore)) * chartArea.height;
...
// Draw move axis
const drawMoveAxis = (ctx: CanvasRenderingContext2D, chartArea: any, dataPoints: any[]) => {
  ctx.fillStyle = '#666';
  ctx.font = '10px Arial';
  ctx.textAlign = 'center';
...
// Draw score line
const drawScoreLine = (ctx: CanvasRenderingContext2D, chartArea: any, dataPoints: any[], minScore: number, scoreRange: number) => {
  ...
  dataPoints.forEach((point, index) => {
    if (point.score !== null && point.score !== undefined) {
      ...
      const y = chartArea.y + chartArea.height - ((point.score - minScore) / scoreRange) * chartArea.height;
...
// Draw data points
const drawDataPoints = (ctx: CanvasRenderingContext2D, chartArea: any, dataPoints: any[], minScore: number, scoreRange: number) => {
  const xStep = chartArea.width / Math.max(1, dataPoints.length - 1);
  ...
  dataPoints.forEach((point, index) => {
    if (point.score !== null && point.score !== undefined) {
      ...
      let color = '#666';
      if (point.score > 100) color = '#c62828'; // Red advantage
      else if (point.score < -100) color = '#2e7d32'; // Black advantage
      else if (point.score > 50) color = '#ef5350'; // Slight red advantage
...
watch([() => props.history, () => props.currentMoveIndex], () => {
  nextTick(() => {
    drawChart();
  });
}, { deep: true });
