      ...
      <div class="capture-section">
        ...
        <div class="captured-pieces">
          ...
          >
            <img
              :src="getPieceImageUrl(piece.name)"
              :alt="piece.name === 'unknown' ? $t('positionEditor.pieces.unknown') : piece.name"
              :class="['captured-piece-img', { 'unknown': piece.name === 'unknown' }]"
            />
          </div>
...
const parseUciExtended = (uciMove: string, movingSide: 'red' | 'black') => {
  ...
  if (extension.length === 1) {
    ...
    console.log(`Single char extension: char="${char}", isUpperCase=${isUpperCase}, charSide="${charSide}"`)
    ...
    console.log(`Two char extension: flip="${extension[0]}", capture="${extension[1]}"`)
    ...
    if (captureChar) {
      ...
      if (isHumanMove) {
        ...
        if (isHumanVsAiMode.value) {
          ...
          console.log(`Added dark piece capture to opponentCaptured: unknown (hidden from human)`)
        } else {
          // In normal mode, show what was captured
          const pieceName = getPieceNameFromChar(captureChar)
          if (pieceName) {
            opponentCaptured.push({ name: pieceName, moveNumber })
            console.log(`Added dark piece capture to opponentCaptured: ${pieceName}`)
          }
        }
        captureHandled = true
      }
    ...
    if (basicCapture) {
      ...
      if (isHumanMove) {
        ...
        if (!captureHandled) {
          ...
          console.log(`Added revealed piece capture to myCaptured: ${basicCapture}`)
        } else {
          console.log(`Basic capture detected but already handled via UCI extension: ${basicCapture}`)
        }
      } else {
        ...
        if (captureHandled) {
          ...
          if (lastEntry && lastEntry.name === 'unknown' && lastEntry.moveNumber === moveNumber) {
            lastEntry.name = basicCapture
            console.log(`Replaced unknown capture with revealed piece: ${basicCapture}`)
          }
        } else {
          opponentCaptured.push({ name: basicCapture, moveNumber })
          console.log(`Added revealed piece capture to opponentCaptured: ${basicCapture}`)
        }
      }
...
const checkBasicCapture = (uciMove: string, moveIndex: number): string | null => {
  ...
  if (moveIndex === 0) {
    fenBefore = gameState.initialFen || 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1'
  } else {
  ...
  try {
    ...
    if (rows[toRow]) {
      ...
      for (let i = 0; i < targetRow.length; i++) {
        ...
        console.log(`Position ${i}: char='${char}', current col=${col}, target col=${toCol}`)
...
const opponentCapturedPieces = computed(() => parseCaptures().opponentCaptured)
</script>
